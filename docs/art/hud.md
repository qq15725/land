...
| 区域 | 锚点 | 内容 |
|------|------|------|
| ① 角色信息条 | 左上 | 头像 + 名字 + Lv + **3 条进度条**（HP / MP / FP） |
...
| ⑦ 快捷栏 | 底部居中 | **上方经验条（XP，唯一显示位）+ 中央等级徽章 + 9 格物品栏** |
...
- 右侧上排：名字 Label 空槽（132, 12, 168×20）+ Lv 徽章空槽（300, 12, 16×20）
- 右侧下方 **3 条进度条空槽**（关键 ⚠️），每条 176×14 px：
  - 行 1：HP 空槽，位置 (128, 44)
  - 行 2：MP 空槽，位置 (128, 66)
  - 行 3：FP 空槽，位置 (128, 88)

> **关键约束**：进度条槽位**只画凹陷边框 + 暗背景**，**不要画任何彩色填充**！实际填充由代码动态绘制圆角胶囊（Panel + StyleBoxFlat），尺寸严格匹配 176×14、左上角 (128, 44/66/88)。任何画在槽位内的色块都会与代码 fill 重叠冲突。
>
> 同理 名字栏 / Lv 徽章 也只画"凹陷暗框"，文字由代码渲染。
>
...
| 元素 | 尺寸 | 说明 |
|------|------|------|
| 经验条**空槽**（XP，唯一显示位） | 584×20 px，起点 (28, 16) | **只画凹陷暗框，无彩色填充**，代码画绿色 fill |
| 等级徽章空槽 | 40×28 px | 内嵌经验条中点，凹陷暗框，数字由代码绘制 |
| 槽位容器 9-patch | 576×80 px，角 16px | 可横向拉伸适配 9 格 |
| 物品格 | 64×64 px × 9 | 等距横排 |
| 选中态高亮 | 80×80 px，9-patch 角 16px | 覆盖物品格外延 8px，与参考图选中格高亮一致 |
| 数量徽章 | 24×16 px | 置于格子右下 |

> **关键约束**：经验条槽位**不画彩色填充**，代码动态画圆角胶囊 fill 在 (28, 16, 584×20) 区域。
>
> 经验条统一只在这里显示，左上角色信息条（§①）不再放经验，左上保留 HP/MP/FP 3 条 + Lv 数字。
>
