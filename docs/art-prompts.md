...
Stardew Valley 风格：明亮温暖的配色，柔和的像素轮廓，阳光乡村氛围。避免黑暗、哥特、压抑元素。
...
| 环境物件 | 视物件而定 | 1列 × 3行 | 第1帧正常，第2帧受击，第3帧破坏消失 |
| 建筑 | 48×48 px | 单帧静态 | 无动画 |
...
```
pixel art sprite sheet, transparent background, 2.5D isometric view,
Stardew Valley art style, soft outlines, warm sunny colors, cheerful rural aesthetic.
...
Pixel art, game asset, no background, clean sprite sheet grid
```
...
```
pixel art sprite sheet, transparent background, 2.5D isometric view,
Stardew Valley art style, soft outlines, warm colors, cute not scary aesthetic.
...
精灵表布局：**1列 × 3行**（正常 / 受击 / 破坏），透明背景，PNG 导出，pivot 在底边中心。

### 提示词模板

```
pixel art sprite sheet, transparent background, 2.5D top-down isometric view,
Stardew Valley art style, soft outlines, bright natural colors, warm sunlit feel.

Sprite sheet layout: 1 column × 3 rows.
...
All rows same canvas size, consistent style.
...
| `grass` | bright fresh green grass tuft（静态，无破坏帧） | 16×16 | `assets/sprites/environment/grass.png` |
| `berry_bush` | leafy green bush with vibrant red berries | 32×32 | `assets/sprites/environment/berry_bush.png` |
| `dead_tree` | pale bare branches, gentle look（装饰，无破坏帧） | 24×48 | `assets/sprites/environment/dead_tree.png` |
| `mushroom` | cheerful red cap with white spots | 16×24 | `assets/sprites/environment/mushroom.png` |

---

## 建筑（静态精灵）

48×48 px，2.5D 斜视角，pivot 在底边中心，透明背景，PNG 导出。

### 提示词模板

```
pixel art sprite, transparent background, 2.5D isometric view,
Stardew Valley art style, soft outlines, warm cozy colors, sunny rural village aesthetic.
Single static image, 48x48 pixels.

Subject: {在此填写建筑描述，如 "wooden workbench with tools on top, warm wood tones"}

Pixel art, game asset, no background
```
...
| id | 描述 | 文件路径 |
|----|------|----------|
| `workbench` | wooden workbench with tools on top, warm wood tones | `assets/sprites/buildings/workbench.png` |
| `storage_chest` | wooden storage chest, closed, light brown with metal clasp | `assets/sprites/buildings/storage_chest.png` |
| `cooking_pot` | stone cooking pot over small orange fire, cozy feel | `assets/sprites/buildings/cooking_pot.png` |
| `farm_plot` | tilled farm plot with moist dark soil rows | `assets/sprites/buildings/farm_plot.png` |
| `trading_post` | wooden trading post booth with colorful sign, inviting look | `assets/sprites/buildings/trading_post.png` |
...
Stardew Valley style, soft outlines, warm bright colors.
Grid layout, each icon 16x16 pixels, {N} columns × {M} rows.

Icons (left to right, top to bottom):
{逐个列出图标描述}

Pixel art, game asset icons, clean grid, no background
```
...
Pixel art, game asset icons, clean grid, transparent background
```
...
风格：Stardew Valley 同款温暖木质 + 羊皮纸质感，圆角描边，奶油米色底，棕色边框。在 Godot 中通过 `StyleBoxTexture`（9-patch）或 `Theme` 全局应用。
...
Stardew Valley style, warm rustic aesthetic, cozy wood and parchment textures,
