...
- [x] F7 动物围栏自动化接口（`Animal.auto_feed` + 抽取器收集 pen 半径内产出掉落物）
- [x] F8 分流 / 过滤（`Splitter` 轮流三向分流 + `FilterConveyor` 锁定首个物品类型分拣）
- [x] F9 生产线总览 UI（`AutomationOverview`：U 键开关，节点计数 + 传送带物品数 + 各类型统计）
- [N/A] F10 能源系统（已决策"无能源"，不实施能耗约束）
...
- 开局牵引：选定职业发放开局工具包（斧 + 镐 + 木板 + 熟胡萝卜，`PlayerActions._grant_starter_kit`，读档不重复）
