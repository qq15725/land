...
- [ ] A1 快捷栏（1-9 数字键直接选物品，hotbar 显示在 HUD 底部）
- [ ] A2 死亡惩罚 + 重生点（家/床作为重生位置；死亡掉落部分背包）
- [ ] A3 自动保存（按时间间隔 / 进入新天 / 关键事件触发）
...
**下一步建议路线**：
- 若优先"修体验细节"：A1 快捷栏 → A3 自动保存 → A2 死亡重生
- 若优先"内容更厚"：B1 货币 → B2 钓鱼 → B4 成就图鉴
- 若优先"修存量"：C1 + C2 + C3（三个轻量 bug 一起做）
- 若优先"加料"：D1 装饰建筑 → D2 多功能建筑（井 / 烤炉 / 铁砧）
